...
The default parameters are deliberately conservative in how they treat non-attention benefit. The virtual PIM bandwidth is set above the GPU roofline anchor to encode a memory-centric advantage, but the virtual PIM throughput is kept far below GPU-class compute throughput so that compute-dominant regions do not inherit unrealistic gains. Likewise, the attention bandwidth bonus is meant only to reflect stronger long-context KV pressure, while the MLP and non-attention penalties are explicitly chosen to avoid overestimating benefit outside attention-heavy regions.
...
\input{tables/final_tables/backend_measured_vs_modeled.tex}
\input{tables/final_tables/backend_parameters.tex}
...
\subsection{Practical Two-Stage Policy}

The empirical findings in this paper naturally suggest a simple practical policy: use context as a coarse gate, use attention labeling only to generate candidates, refine the decision with KV-related signals, and add hysteresis to avoid rapid oscillation. We instantiate this idea as \texttt{context\_kv\_hysteresis}, which keeps separate on/off thresholds for KV bytes, KV bytes per token, memory-pressure proxy, and an aggregate boundary score.

\begin{algorithm}[t]
\caption{Context-Gated KV-Refined Hysteresis Policy}
\label{alg:context-kv-hysteresis}
\begin{algorithmic}[1]
\REQUIRE Region record $r_t$, previous state $a_{t-1}\in\{\texttt{gpu},\texttt{pim}\}$
\IF{$r_t$ is not an attention-region candidate}
    \RETURN \texttt{gpu}
\ENDIF
\IF{$\mathrm{context}(r_t) < C_{\min}$}
    \RETURN \texttt{gpu}
\ENDIF
\STATE Compute boundary score $s_t$ from context length, KV bytes, KV bytes/token, and memory-pressure proxy
\IF{$a_{t-1}=\texttt{gpu}$}
    \IF{$s_t \ge S_{\mathrm{on}}$ and KV-related on-thresholds are satisfied}
        \RETURN \texttt{pim}
    \ELSE
        \RETURN \texttt{gpu}
    \ENDIF
\ELSE
    \IF{$s_t \ge S_{\mathrm{off}}$ and KV-related off-thresholds are satisfied}
        \RETURN \texttt{pim}
    \ELSE
        \RETURN \texttt{gpu}
    \ENDIF
...
We do not claim that this policy is globally optimal or fully calibrated. Its thresholds are fixed conservatively to prioritize false-positive control and lower switching rather than maximum average speedup. This makes it a practical translation of the paper's characterization results rather than a learned scheduler.
